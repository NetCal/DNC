...
public class PmooAnalysis extends AbstractAnalysis implements Analysis {
	...
	public static ServiceCurve getServiceCurve(Path path, List<Flow> cross_flow_substitutes) {
		...
		ServiceCurve beta_total = CurvePwAffineFactory.getCurveFactory().createZeroService();
	 ...
	 *
	 * @param path
	 *            The tandem of servers the left-over service curve holds for.
	 * @param service_curves
	 *            The service curves on the path.
	 * @param cross_flow_substitutes
	 *            Flows representing a group of segregated flows.
	 * @param flow_tb_iter_map
	 *            Defines which token bucket component of a flow's arrival bound to
	 *            use.
	 * @param server_rl_iters
	...
	protected static ServiceCurve computePartialPMOOServiceCurve(Path path, ServiceCurve[] service_curves,
		...
		if (R == NumFactory.getNumFactory().getPositiveInfinity()) {
			return CurvePwAffineFactory.getCurveFactory().createDelayedInfiniteBurst(T);
		}

		return CurvePwAffineFactory.getCurveFactory().createRateLatency(R, T);
	}
	...
	private Set<ServiceCurve> getServiceCurvesFP(Flow flow_of_interest, Path path, Set<Flow> flows_to_serve)
		...
		if (betas_e2e.isEmpty()) {
			betas_e2e.add(CurvePwAffineFactory.getCurveFactory().createZeroService());
		}
		return betas_e2e;
	}
