...
public abstract class ArrivalBoundDispatch {
    ...
    private static Set<ArrivalCurve> singleFlowABs(AnalysisConfig configuration, ArrivalCurve alpha, Set<ServiceCurve> betas_lo) throws Exception {
        ...
        for (ServiceCurve beta_lo : betas_lo) {
            arrival_bounds_f.add(MinPlusDispatch.deconvolve(alpha, beta_lo, configuration.tbrlDeconvolution()));
        }

        return arrival_bounds_f;
    }

    private static Set<ArrivalCurve> getPermutations(Set<ArrivalCurve> arrival_curves_1, Set<ArrivalCurve> arrival_curves_2) {
    ...
    private static void addArrivalBounds(AnalysisConfig configuration, Set<ArrivalCurve> arrival_bounds_to_merge, Set<ArrivalCurve> arrival_bounds) {
    ...
    private static void addArrivalBounds(AnalysisConfig configuration, ArrivalCurve arrival_bound_to_merge, Set<ArrivalCurve> arrival_bounds) {
        if (configuration.arrivalBoundMethods().size() == 1) { // In this case there can only be one arrival bound
