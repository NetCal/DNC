...
public class Path {
	...
	{
		ServiceCurve service_curve_total = ServiceCurve.createZeroDelayInfiniteBurst();
 	...
 	/**
	 * Returns the convolution of the maximum service curves of all servers on
	 * the given lnik path <code>path</code> that have the useGamma flag set.<br>
	 * If a server either has no maximum service curve set or useGamma is disabled,
	 * calculations take place with the default maximum service curve,
	...
	{
		MaxServiceCurve gamma_total = MaxServiceCurve.createZeroDelayInfiniteBurst();
		for ( Server s : servers )
		{
	...
	/**
	 * Returns the convolution of the maximum service curves of all servers on
	 * the given link path <code>path</code> that have the useExtraGamma flag set.<br>
	 * If a server either has no maximum service curve set or useExtraGamma is disabled,
	 * calculations take place with the default maximum service curve,
	 * i.e., the zero delay burst curve, so the result will not be influenced.
	 * 
	 * @return The convolved curve
	...
	{
		MaxServiceCurve extra_gamma_total = MaxServiceCurve.createZeroDelayInfiniteBurst();
		for ( Server s : servers )
		{
...
//		extra_gamma_total.removeLatency(); // Already done by s.getExtraGamma()
	...
	/**
	 * Returns the convolution of the maximum service curves of all servers on
	 * the given link path <code>path</code><br>
	 * If a server has no maximum service curve, calculations take place with 
	 * the default maximum service curve, i.e., the zero delay burst curve, 
	...
	{
		MaxServiceCurve max_service_curve_total = MaxServiceCurve.createZeroDelayInfiniteBurst();
		for ( Server s : servers )
		{
	...
	@Override
	public int hashCode() {
		return (int) Arrays.hashCode( path_servers.toArray() )
					* Arrays.hashCode( path_links.toArray() );
	}
