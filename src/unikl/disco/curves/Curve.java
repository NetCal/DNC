...
public class Curve {
	...
	protected void initializeDelayedInfiniteBurst( Num latency ) {
		...
		getSegment( 1 ).x        = Num.max( NumFactory.getZero(), latency );
	...
	public boolean isDiscontinuity( int i ) {
		return (i+1 < segments.length && ( Num.abs( Num.sub( segments[i+1].x, segments[i].x ) ) ).less( NumFactory.getEpsilon() ) );
	}
	...
	public boolean isRealDiscontinuity( int i ) {
		return (isDiscontinuity(i) && ( Num.abs( Num.sub( segments[i+1].y, segments[i].y ) ) ).ge( NumFactory.getEpsilon() ) );
	}
	...
	public boolean isUnrealDiscontinuity( int i ) {
		return (isDiscontinuity(i) && ( Num.abs( Num.sub(segments[i+1].y, segments[i].y ) ) ).less( NumFactory.getEpsilon() ) );
	}
	...
	public void computeYs(int start) {
		...
		for (int i = start; i < segments.length; i++) {
			dx = Num.sub( segments[i].x, segments[i-1].x );
	...
	public void beautify() {
		...
		while(i < segments.length-1) {
			...
			if ( Num.abs( firstArg ).less( NumFactory.getEpsilon() )
					&& Num.abs( secondArg ).less( NumFactory.getEpsilon() ) ){
	...
	public boolean isConvexIn( Num a, Num b ) {
		...
		for (int i = i_start; i <= i_end; i++) {
			...
			Num gradient;
			if (i < segments.length-1) {
				gradient = Num.sub( segments[i+1].y, segments[i].y );
				gradient.div( Num.sub( segments[i+1].x, segments[i].x ) );
			} else {
	...
	public Num getLatency() {
		...
		for (int i = 0; i < segments.length; i++) {
			Num y0 = segments[i].y;
			if (y0.less( NumFactory.getZero() ) && y0.greater( Num.negate( NumFactory.getEpsilon() ) ) ) {
